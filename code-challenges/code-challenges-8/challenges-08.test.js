...
const filterStringsWithVowels = (arr) => {
  ...
  return arr.filter(word => {
    if(word.match(vowels)) return word;
  });
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(val => {
    if(forbiddenValues.includes(val) === false) return val; // if condition checks for the number not in first array and then returns the one not there
  });
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // one liner!  previous could be done in one line too, go back and check later
