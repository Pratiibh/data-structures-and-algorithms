...
const returnNames = (arr) => {
  let result = [];
  const reducer = (testA,testB) => {
    result.push(testB.name);
    return testA;
  }
...
const reversedString = (arr) => {
  let arrayConv = arr.split('');
  let result = [];
  arrayConv.reduce((testA,testB) => {
    result.unshift(testB);
    return testA;
  }, 0); // look back at this later.  don't remember why the group did like this
  result = result.join('');
  return result;
};
...
const countNumberOfChildren = (arr) => {
  let children = arr.reduce((testA, testB) => {
    //test b holds children object
    if (testB.children) testA += testB.children.length;
    return testA;
  }, 0); // same formatting
  return children;
};
