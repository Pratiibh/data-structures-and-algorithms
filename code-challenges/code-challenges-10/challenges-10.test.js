...
const count = (target, input) => {
  let result = [];
  const arraySum = (sum, val) => sum + val;
  // ^ "erase em" lol
  const filter = num => num === target;

  if(input.length){
    for (let i in input){
      let arr = input[i];
      let match = arr.filter(filter);
      result.push(match.length);
    }
    return(result.reduce(arraySum));
  }
  return 0;
};
...
const totalSum = (input) => {
  const arraySummer = (sum, val) => sum + val;
  let everyArraySum =[];
  // group has vars diff names, don't mix up when asking q's
  for (let i in input){
    let singleArraySum = input[i].reduce(arraySummer);
    everyArraySum.push(singleArraySum);
  }

  const result = everyArraySum.reduce(arraySummer)
  return result;
};
    ...
    let result = input.map(arr => {
        return arr.filter(val => {
          if(typeof val === 'number'){
              // typeof used to find what type of data structure it is
            if(val % 5 === 0) return true;
          }
        }).map(keptNumber => {
          let result = 2 ** keptNumber;
          return result;
        });
      });
      return result;
...
let findMaleAndFemale = (data) => {
  // nah lol assignment already finished call it good
};
